...
class ApproachDebris(RobotController):
    def __init__(self, parent: RobotController = None):
        ...
        self.pid_error_threshold_x = (
            0.02  # with the adaptive gripper, we can be more "free" here as a default
        )
        self.pid_error_threshold_y = 0.02
        self.register_property(
            "pid_error_threshold_x",
            descr="Decide if target is reached if horizontal errors are below this threshold, coordinates in relative position (0..1)",
        )
        self.register_property(
            "pid_error_threshold_y",
            descr="Decide if target is reached if vertical errors are below this threshold, coordinates in relative position (0..1)",
        )
        ...
        self.register_property(
            "kp_x",
            default_value=default_kp_x,
            descr="PID controller proportial gain for horizontal/rotational errors",
        )
        self.register_property(
            "kp_y",
            default_value=default_kp_y,
            descr="PID controller proportial gain for vertical/distance errors",
        )
        self.register_property(
            "setpoint_x",
            default_setpoint_x,
            descr="Horizontal taget position in relative image coordinates, e.g. 0.25 is left quarter of image; 0.5 is image center.",
        )
        self.register_property(
            "setpoint_y",
            default_setpoint_y,
            descr="Vertical target position in rleative image coordinates, e.g. 0.25 is lower quarter of image; 0.5 is image center.",
        )

        # Register member variables of this class, will be update automatically in case of user interaction
        self.register_property(
            "output_enabled", descr="Output estimated control values to motors."
        )
        self.register_property("pid_debug")

        # Create pid controller:
        self.ctrl = PIDController(
            setpoint_x=default_setpoint_x,
            setpoint_y=default_setpoint_y,
            kp=(default_kp_x, default_kp_y),
        )
    ...
    def update(self, robot: RobotInterface, detections: List[BoxDef] = None) -> bool:
        ...
        if trash_to_follow is not None and len(trash_to_follow) > 0:
            ...
            dir_command = [
                (
                    min(max(n, -100), -self.minimum_output)
                    if n < -self.minimum_output
                    else max(min(n, 100), self.minimum_output) if n > 15 else 0
                )
            ...
            if self.debug:
                logger.debug(
                    f"dir_command: {dir_command}, dir_error: {dir_error_x, dir_error_y} target_arrival_frames: {self.target_arrival_frames}"
                )
            ...
            if (
                abs(dir_error_x) < self.pid_error_threshold_x
                and abs(dir_error_y) < self.pid_error_threshold_y
            ):
