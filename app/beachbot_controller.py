...
parser.add_argument(
    ...
    default=False,
    action="store_true",
    help="Execute in simulation instead of on real robot",
)
parser.add_argument(
    "--cfgload",
    default=False,
    action="store_true",
    help="Reload stored configuration on startup",
)
parser.add_argument(
    "--cfgfile",
    type=str,
    ...
    try:
        if robot is not None:
            ...
            def update_prop(robot: RobotInterface, name, val):
                ...
                origstate[name] = val
                modified_state = {
                    n: robot.get_property(n) for n in robot.list_property_names()
                }
            ...
            for prop in robot.list_property_names():
                ...
                if value_subclass:
                    if value_subclass in prop_classes:
                        ...
                        targetelement_parent = ui.expansion(
                            "robot." + value_subclass + ":", icon="tune"
                        ).classes("w-full justify-between no-wrap")
                        prop_classes[value_subclass] = targetelement_parent

                    with targetelement_parent:
                        targetelement = ui.row().classes(
                            "w-full justify-between no-wrap"
                        )
                else:
                ...
                with targetelement:
                    ...
                    if type(value) == str:
                        ...
                        ui.input(
                            ...
                            value=value,
                            on_change=lambda e, p=prop: update_prop(robot, p, e.value),
                        )
                        ...
                        if (
                            ...
                            ui.slider(
                                min=value_bounds[0],
                                max=value_bounds[1],
                                step=(value_bounds[1] - value_bounds[0]) / 255.0,
                                value=value,
                                on_change=lambda e, p=prop: update_prop(
                                    robot, p, float(e.value)
                                ),
                            ).props("label-always")
                        else:
                            tooltipelement = ui.label("robot." + prop + ":")
                    ...
                    with targetelement:
                        with ui.context_menu():
                            ...
                            ui.menu_item(
                                "Hide",
                                auto_close=False,
                                on_click=lambda _, elstr=prop: hide_el(elstr),
                            )
                            ui.separator()
                            ui.menu_item(
                                "Reset",
                                auto_close=False,
                                on_click=lambda _, elstr=prop: reset_el(elstr),
                            )
...
def reload_files():
    ...
    with selector:
        for dirpath, dirnames, filenames in walk(
            Path(VideoWriterOpenCV.get_base_path())
        ):
...
def update_detection(robot: RobotInterface):
    ...
    if boxes is not None:
        for b in boxes:
            boxsvg += add_imgbox(
                b.left, b.top, b.w, b.h, b.class_name, conf_value=b.confidence
            )
    return frame, boxsvg
    ...
    with ui.tab_panel(one):
            ...
            with ui.column():
                with ui.row().classes("w-full"):
                    with ui.dropdown_button(
                        "Select Model", auto_close=True
                    ) as model_selector:
                        ui.item(
                            "Detection Off", on_click=lambda: toggle_detection(None)
                        )
                        for model in model_list:
                            ui.item(
                                str(model.__name__),
                                on_click=lambda m=model: toggle_detection(m),
                            )
                    ui.space()
                    with ui.dropdown_button(
                        "Select Controller", auto_close=True
                    ) as controller_selector:
                        controller_list
                        ui.item("Control Off", on_click=lambda: toggle_controller(None))
                        for controller in controller_list:
                            ui.item(
                                str(controller.__name__),
                                on_click=lambda m=controller: toggle_controller(m),
                            )
        ...
        with ui.splitter().classes("w-full h-full") as splitter:
            with splitter.before:
                ...
                tab_panel_ctrl = ui.tab_panels(tabs_ctrl, value=one_ctrl).classes(
                    "w-full"
                )
                with tab_panel_ctrl.classes("w-full h-full"):
                    ...
                    with ui.tab_panel(two_ctrl):
                        ...
                        ui.button(
                            "Save Calib Pose", on_click=lambda x: arm_calib_save()
                        )
                    ...
                    with ui.tab_panel(three_ctrl):
                        ...
                        with ui.row().classes("w-full justify-between no-wrap"):
                            ui.label("x:")
                            cart_x_slider = ui.slider(
                                min=-200,
                                max=200,
                                step=1,
                                value=0.0,
                                on_change=lambda x: arm_action_cartesian(),
                            ).props("label")
                        with ui.row().classes("w-full justify-between no-wrap"):
                            ui.label("y:")
                            cart_y_slider = ui.slider(
                                min=-200,
                                max=200,
                                step=1,
                                value=0.0,
                    ...
                    with ui.tab_panel(four_ctrl).classes("w-full h-full border"):
                        with ui.row():
                            ...
                            ui.button(
                                "Refresh", on_click=lambda _: ui_config_panel.refresh()
                            )
            ...
            with splitter.after:
                ...
                tab_panel_view = ui.tab_panels(tabs_view, value=one_view).classes(
                    "w-full"
                )
                with tab_panel_view.classes("w-full h-full"):
                    ...
                    with ui.tab_panel(two_view):
                        controller_plot = (
                            ui.line_plot(n=6, limit=300, figsize=(9, 6), update_every=5)
                            .with_legend(
                                [
                                    "Motor1",
                                    "Motor2",
...
def store_config():
    global hidden_properties
    robot.store_properties(
        robot_config_filename,
        payload={"gui.hidden_properties": ",".join(hidden_properties)},
    )
...
def provide_config():
    ...
    return JSONResponse(
        content=storage,
        headers={"Content-Disposition": f'attachment; filename="config.json"'},
    )
...
ui.run(
    ...
    show=False,
    title="Beachbot",
    favicon=str(get_asset_path() / "beachbot_128x128.png"),
)
